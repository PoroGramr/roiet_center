...
public class StudentService {
 ...
 public List<Summary> list(String q,Student.Status status,Long teamId){
  ...
  return students.search(blankToNull(q),status,teamId).stream().map(s->summary(s,current.get(s.getId()))).toList();
 }
 ...
 @Transactional public Summary move(Long id,MoveTeamRequest r){
  ...
  if(current!=null) current.endOn(r.movedAt().minusDays(1));
