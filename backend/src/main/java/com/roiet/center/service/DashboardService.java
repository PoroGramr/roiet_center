...
public class DashboardService {
 ...
 public Response get(){
  LocalDate today=LocalDate.now(); var todaySessions=sessions.findBySessionDateAndDeletedFalse(today); var todayDto=todaySessions.stream().map(s->{var rows=attendance.findBySession(s.getId());return new TodaySession(s.getId(),s.getTeam().getId(),s.getTeam().getName(),credited(rows),rows.size(),s.getStartTime());}).toList();
