...
public class SessionService {
 ...
 public List<Summary> list(Long teamId){return sessions.findRecent(teamId).stream().map(this::summary).toList();}
 ...
 private Summary summary(ClassSession s){var rows=attendances.findBySession(s.getId());return new Summary(s.getId(),s.getTeam().getId(),s.getTeam().getName(),s.getSessionDate(),s.getTitle(),s.getContent(),rows.stream().filter(x->policy.countsAsPresent(x.getStatus())).count(),rows.size());}
