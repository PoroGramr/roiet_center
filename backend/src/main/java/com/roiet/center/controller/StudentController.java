...
public class StudentController {
 ...
 @PostMapping @ResponseStatus(HttpStatus.CREATED) public Summary create(@Valid @RequestBody SaveRequest r){return service.create(r);}
 @GetMapping("/{id}") public Detail get(@PathVariable Long id){return service.get(id);}
 @PatchMapping("/{id}") public Summary update(@PathVariable Long id,@Valid @RequestBody SaveRequest r){return service.update(id,r);}
 @PostMapping("/{id}/move-team") public Summary move(@PathVariable Long id,@Valid @RequestBody MoveTeamRequest r){return service.move(id,r);}
}
